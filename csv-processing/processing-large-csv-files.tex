...
def isAscending(l):
  ...
  return True    
\end{lstlisting}
对于Predicate的接口形式，只要使用起来方便即可。如果过滤数据的时候用到了很多个
Predicate，每个Predicate最好按类型统一接口，这样可以用一致的方法处理；
这些Predicate最好组合成一个Predicate，能一次做完全部的测试，以便调用者在使用时，
